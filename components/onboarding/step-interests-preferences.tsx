...
export type InterestsPreferencesData = z.infer<typeof finalStepSchema>;

interface StepInterestsPreferencesProps {
    defaultValues: Partial<InterestsPreferencesData>;
    onBack: () => void;
    onSubmit: (data: InterestsPreferencesData) => void;
    isSubmitting: boolean;
}

export default function StepInterestsPreferences({
    defaultValues,
    onBack,
    onSubmit,
    isSubmitting,
}: StepInterestsPreferencesProps) {
    ...
    const {
        ...
        clearErrors,
        formState: { errors },
    } = useForm<InterestsPreferencesData>({
    ...
    const syncCommunitySelections = useCallback((map: Map<number, Set<number>>) => {
    ...
    useEffect(() => {
        async function loadTaxonomy() {
            try {
                ...
                if (!communityRes.ok || !industryRes.ok) {
    ...
    useEffect(() => {
        ...
        (defaultValues.communitySelections || []).forEach((selection) => {
            if (!map.has(selection.communityId)) {
                map.set(selection.communityId, new Set());
            }
            map.get(selection.communityId)!.add(selection.subCommunityId);
        });
        setSelectedCommunities(map);
        syncCommunitySelections(map);
    }, [defaultValues.communitySelections, syncCommunitySelections]);
    ...
    const currentSubIndustries = useMemo(
        () => industries.find((industry) => industry.id === selectedIndustryId)?.sub_industries || [],
    ...
    useEffect(() => {
        if (Number(selectedSubIndustryId) > 0) {
            clearErrors("subIndustryId");
        }
    ...
    const industrySelectProps = register("industryId", {
        valueAsNumber: true,
        onChange: () => {
            clearErrors(["industryId", "subIndustryId"]);
        },
    });
    ...
    const selectFrequency = (value: string) => {
        if (!FREQUENCIES.some((frequency) => frequency.value === value)) {
            return;
        }
    ...
    return (
        ...
        >
            <div>
                <h2 className="text-2xl font-bold text-zinc-900">Your interests</h2>
                    ...
                    return (
                        ...
                        >
                            <label className="block text-sm font-medium text-zinc-500 mb-2">
                                Sub-communities for <span className="text-[#0AB996]">{community.name}</span>
                            </label>
                            <div className="flex flex-wrap gap-2">
                                {community.sub_communities.map((subCommunity) => {
                                    const isSelected = subCommunitySet.has(subCommunity.id);
                                    return (
                                        <button
                                            key={subCommunity.id}
                                            type="button"
                                            onClick={() => toggleSubCommunity(communityId, subCommunity.id)}
            ...
            <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-1">
                    ...
                    {errors.industryId && <p className="text-red-500 text-xs">{errors.industryId.message}</p>}
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            {currentSubIndustries.map((subIndustry) => (
                    ...
                    {errors.subIndustryId && <p className="text-red-500 text-xs">{errors.subIndustryId.message}</p>}
                </div>
            </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {FREQUENCIES.map((frequency) => {
                            const isActive = currentFrequency === frequency.value;
                            return (
                                <button
                                    key={frequency.value}
                                    type="button"
