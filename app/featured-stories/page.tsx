...
export default function FeaturedStoriesPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const url = `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][$eq]=Featured Stories&populate=*&pagination[pageSize]=100&sort=Date:desc`;
                ...
                if (json.data) {
                    const formattedData = json.data.map((item: any) => {
                        ...
                        let excerptText = "Featured story from ENERGDIVE.";
    ...
    if (articles.length === 0) return (<div className="min-h-screen bg-white"><Header /><div className="flex items-center justify-center pt-32 text-gray-500">No featured stories found.</div></div>);
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex-1">
                                <h1 className="text-3xl sm:text-5xl md:text-7xl lg:text-[8vw] font-black tracking-tighter leading-[0.85] uppercase italic">
                                    <span className="text-[#00A651]">Featured</span> Stories
                                </h1>
                            </motion.div>
                            <div className="lg:w-1/3 border-l-2 border-black pl-6 py-2">
                                <p className="text-sm md:text-base font-bold tracking-widest text-gray-500 leading-relaxed italic">
                                    Signature long-form narratives from ENERGDIVE — curated stories that define the energy transition.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="container mx-auto px-6 lg:px-12 flex items-center">
                        <div className="flex gap-16 items-center animate-marquee whitespace-nowrap text-[11px] font-bold text-gray-400 uppercase">
                            {articles.slice(0, 10).map((a, i) => (
                                <Link key={i} href={`/featured-stories/${a.slug}`} className="hover:text-black flex items-center gap-3 transition-colors">
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-12 py-10 sm:py-16">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mt-10 mb-24">
                        <div className="lg:col-span-8 group">
                            <Link href={`/featured-stories/${heroArticle.slug}`} className="block">
                        ...
                        <div className="lg:col-span-4 flex flex-col border-t lg:border-t-0 lg:border-l border-gray-200 pt-10 lg:pt-0 lg:pl-10">
                            <h3 className="text-sm md:text-base font-black uppercase tracking-[4px] mb-10 border-b-2 border-black pb-4">Recent Featured Stories</h3>
                            <div className="space-y-8">
                                {topInsights.map((item, idx) => (
                                    <Link href={`/featured-stories/${item.slug}`} key={idx} className="flex gap-5 group items-start">
                    ...
                    {visibleItems.length > 0 && (
                        <section className="mb-32">
                            <div className="flex items-center gap-4 mb-12">
                                <h2 className="text-4xl font-black uppercase italic">Featured Stories <span className="text-[#00A651] not-italic">Archive</span></h2>
                                ...
                                {visibleItems.map((item, idx) => (
                                    <div key={idx} className="group flex flex-col border-t border-gray-100 pt-6 hover:border-black transition-all duration-500">
                                        ...
                                        <div className="flex flex-col flex-1">
                                            ...
                                            <Link href={`/featured-stories/${item.slug}`} className="mt-auto inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest hover:gap-4 transition-all text-black">
