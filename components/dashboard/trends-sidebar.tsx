...
// import { FileBarChart2, Loader2, Plus, X, ChevronDown, User, Calendar, Pencil } from "lucide-react";
...
function CommunityWidget({
    ...
    const editingCommObj = allCommunities.find((c) => c.id === editingId) ?? null;

    // Fetch all communities when needed
    useEffect(() => {
        if (showAdd && !loaded) {
            fetch("/api/master/communities")
                .then((r) => r.json())
        ...
        try {
            const newSelections = currentCommunities.map((c) =>
                c.community_id === communityId
                    ? { communityId: c.community_id, subCommunityId: editSubId }
                    : { communityId: c.community_id, subCommunityId: c.sub_community_id }
            );
            await fetch("/api/user/update-profile", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                                        ...
                                        <div key={`${c.community_id}-${c.sub_community_id}`} className="border-b last:border-0" style={{ borderColor: "var(--dash-border-subtle)" }}>
                                            <div className="flex items-center justify-between px-3 py-2 group hover:bg-black/5 transition-colors">
                                                ...
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    {/* Remove button */}
                                                    <button
                                                        onClick={() => handleRemove(c.community_id, c.sub_community_id)}
                                                        disabled={saving}
                                                        className="p-1 rounded transition-colors hover:bg-red-500/10"
                                                        style={{ color: "var(--dash-text-dim)" }}
                                                        title="Remove community"
