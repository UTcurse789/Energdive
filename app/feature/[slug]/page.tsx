...
async function getRelated(currentSlug: string) {
    const res = await fetch(
        `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][$eq]=Feature&filters[slug][$ne]=${currentSlug}&pagination[limit]=4&populate=*&sort=publishedAt:desc`,
        { cache: "no-store" }
    );
    if (!res.ok) return [];
    const json = await res.json();
            ...
            <main className="pt-20 pb-24">
                <div className="container mx-auto max-w-7xl px-4 sm:px-6 mb-6 sm:mb-8">
                    <nav className="flex items-center gap-1.5 text-xs text-gray-400 font-sans">
                        ...
                        <Link href="/feature" className="hover:text-teal-600 transition-colors">Feature</Link>
                        <ChevronRight className="h-3 w-3" />
