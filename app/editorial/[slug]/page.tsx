...
async function getRelated(currentSlug: string) {
    const res = await fetch(
        `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][$eq]=Editorial&filters[slug][$ne]=${currentSlug}&pagination[limit]=4&populate=*&sort=publishedAt:desc`,
        { cache: "no-store" }
    );
    if (!res.ok) return [];
    const json = await res.json();
...
export async function generateMetadata({
    ...
    return {
        ...
        openGraph: {
            title,
            description,
            url: `https://energdive.com/editorial/${slug}`,
            siteName: "Energdive",
            images: [
                {
                    url: imageUrl,
...
export default async function EditorialDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const article = {
        ...
        tags: normalizedTags,
        category: attrs.type_of_content?.name || attrs.type_of_content?.data?.attributes?.name || "Editorial",
    };
            ...
            <main className="pt-20 pb-24">
                <div className="container mx-auto max-w-7xl px-4 sm:px-6 mb-6 sm:mb-8">
                    <nav className="flex items-center gap-1.5 text-xs text-gray-400 font-sans">
                        ...
                        <Link href="/editorial" className="hover:text-teal-600 transition-colors">Editorial</Link>
                        <ChevronRight className="h-3 w-3" />
                ...
                <div className="container mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 max-w-7xl">
                    <div className="lg:col-span-8">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <ShareButton title={article.title} text={article.excerpt.length ? article.excerpt[0]?.children?.[0]?.text : "Check out this editorial"} className="text-gray-500 hover:text-teal-600 font-medium text-sm border border-gray-200 px-3 py-1.5 rounded-full bg-white hover:bg-gray-50 shadow-sm" />
                        </div>
                        <h1 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-bold leading-[1.08] tracking-tight text-gray-900 mb-4 sm:mb-6">{article.title}</h1>
                        <div className="text-base sm:text-xl text-gray-500 font-serif leading-relaxed mb-6 sm:mb-8 border-l-4 border-teal-500 pl-4 sm:pl-5"><BlocksRenderer content={article.excerpt} /></div>
                        {article.author && (
                        ...
                        <article className="relative"><div className="prose prose-lg max-w-none font-serif text-[18px] leading-[1.95] text-gray-800 prose-headings:font-bold prose-headings:text-gray-900 prose-headings:tracking-tight prose-h2:text-3xl prose-h2:mt-12 prose-h2:mb-4 prose-h2:border-b prose-h2:border-gray-100 prose-h2:pb-3 prose-h3:text-2xl prose-h3:mt-10 prose-h3:mb-3 prose-p:mb-6 prose-a:text-teal-600 prose-a:decoration-teal-300 hover:prose-a:text-teal-800 prose-strong:text-gray-900 prose-blockquote:border-l-teal-500 prose-blockquote:bg-teal-50/30 prose-blockquote:rounded-r-lg prose-blockquote:py-2 prose-img:rounded-lg prose-img:shadow-md prose-li:marker:text-teal-500 first:prose-p:first-letter:text-6xl first:prose-p:first-letter:font-serif first:prose-p:first-letter:font-bold first:prose-p:first-letter:float-left first:prose-p:first-letter:mr-3 first:prose-p:first-letter:mt-1 first:prose-p:first-letter:text-teal-700"><ArticleBody content={article.content} dataBlocks={dataBlocks} /></div></article>
                        {article.tags.length > 0 && (<div className="mt-12 pt-6 border-t border-gray-100"><h4 className="text-xs uppercase tracking-widest text-gray-400 mb-4 font-bold">Tags</h4><div className="flex flex-wrap gap-2">{article.tags.map((tag: any) => (<TagBadge key={tag.slug} name={tag.name} slug={tag.slug} className="bg-teal-50 text-teal-700 px-3 py-1.5 text-xs font-medium uppercase tracking-wider rounded-full border border-teal-100 hover:bg-teal-600 hover:text-white hover:border-teal-600" />))}</div></div>)}
                    </div>
