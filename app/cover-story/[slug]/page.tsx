...
async function getRelated(currentSlug: string) {
    const res = await fetch(
        `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][$eq]=Cover Story&filters[slug][$ne]=${currentSlug}&pagination[limit]=4&populate=*&sort=publishedAt:desc`,
        { cache: "no-store" }
    );
    if (!res.ok) return [];
    const json = await res.json();
...
export default async function CoverStoryDetailPage({
    params,
}: {
            ...
            <main className="pt-20 pb-24">
                <div className="container mx-auto max-w-7xl px-4 sm:px-6 mb-6 sm:mb-8">
                    <nav className="flex items-center gap-1.5 text-xs text-gray-400 font-sans">
                        ...
                        <Link href="/cover-story" className="hover:text-teal-600 transition-colors">Cover Story</Link>
                        <ChevronRight className="h-3 w-3" />
                ...
                <div className="container mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 max-w-7xl">
                    <div className="lg:col-span-8">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <ShareButton title={article.title} text={article.excerpt.length ? article.excerpt[0]?.children?.[0]?.text : "Check out this cover story"} className="text-gray-500 hover:text-teal-600 font-medium text-sm border border-gray-200 px-3 py-1.5 rounded-full bg-white hover:bg-gray-50 shadow-sm" />
                        </div>
                        ...
                        {article.author && (
                            <div className="flex items-center gap-4 mb-10 pb-8 border-b border-gray-100">
                                {article.author.avatar ? (
                                    <Image src={article.author.avatar} width={52} height={52} alt={article.author.name || ""} className="rounded-full ring-2 ring-teal-100" />
                                ) : (
                                    <div className="w-[52px] h-[52px] rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-lg">{article.author.name?.charAt(0) || "A"}</div>
                                )}
                                <div>
                        ...
                        <div className="relative aspect-video mb-12 rounded-xl overflow-hidden shadow-lg shadow-black/10 group">
                            <Image src={article.image} alt={article.title || ""} fill priority className="object-cover transition-transform duration-700 group-hover:scale-[1.02]" />
                        ...
                        {article.tags.length > 0 && (
                            <div className="mt-12 pt-6 border-t border-gray-100">
                                <h4 className="text-xs uppercase tracking-widest text-gray-400 mb-4 font-bold">Tags</h4>
                                <div className="flex flex-wrap gap-2">
                                    {article.tags.map((tag: any) => (
                                        <TagBadge key={tag.slug} name={tag.name} slug={tag.slug} className="bg-teal-50 text-teal-700 px-3 py-1.5 text-xs font-medium uppercase tracking-wider rounded-full border border-teal-100 hover:bg-teal-600 hover:text-white hover:border-teal-600" />
                                    ))}
                                </div>
                            </div>
                        )}
                            ...
                            {latestIssue && (
                                <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
                                    ...
                                    <Link href={`/issues/${latestIssue.slug}`} className="group block">
                                        ...
                                        <h4 className="font-serif font-bold text-gray-900 group-hover:text-teal-600 transition-colors mb-1">{latestIssue.month} {latestIssue.year}</h4>
                                        <span className="inline-flex items-center gap-1 text-xs font-bold text-teal-600 group-hover:gap-2 transition-all">Read Issue <ArrowRight className="h-3.5 w-3.5" /></span>
                                    </Link>
                                </div>
                            )}
                                            ...
                                            return (
                                                <Link key={item.id} href={`/cover-story/${r.slug}`} className="group flex gap-4 rounded-lg p-2 -mx-2 transition-colors hover:bg-gray-50">
                                                    <div className="relative w-24 h-20 shrink-0 overflow-hidden rounded-lg bg-gray-100">
                                                        <Image src={imgUrl} alt="" fill className="object-contain bg-white p-0.5 transition-transform duration-500 group-hover:scale-[1.02]" />
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <h4 className="font-serif font-bold text-sm leading-snug text-gray-900 group-hover:text-teal-600 transition-colors line-clamp-2 mb-1">{r.Title}</h4>
                                                        {itemDate && <DateChip value={itemDate} className="text-[10px]" />}
                                                    </div>
                                                </Link>
                                            );
                                        })}
