...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!onboardingCompleted) {
      const returnTo = `/resource-center/${encodeURIComponent(resource.slug)}?download=true`;
      return NextResponse.json(
        {
    ...
    return NextResponse.json({
      ...
      downloadUrl: `/api/resource-center/download?slug=${encodeURIComponent(resource.slug)}`,
