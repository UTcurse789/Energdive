...
const featuredOpinion = ARTICLES[0]; // Reuse for demo
const opinionList = ARTICLES.slice(1, 4);
                        ...
                        <div className="flex items-center gap-4">
                            {featuredOpinion.author && (
                                <div className="relative w-16 h-16 rounded-full overflow-hidden border-2 border-primary">
                                    <Image src={featuredOpinion.author.avatar} alt={featuredOpinion.author.name} fill className="object-cover" />
                                </div>
                            )}
                            <div>
                                <div className="font-bold text-lg">{featuredOpinion.author?.name}</div>
                                <div className="text-sm text-muted-foreground">{featuredOpinion.author?.bio}</div>
                            </div>
                        </div>
                    </div>
