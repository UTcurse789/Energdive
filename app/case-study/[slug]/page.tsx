...
async function getRelated(currentSlug: string) {
    const res = await fetch(
        `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][$eq]=Case Study&filters[slug][$ne]=${currentSlug}&pagination[limit]=4&populate=*&sort=publishedAt:desc`,
        { cache: "no-store" }
    );
    if (!res.ok) return [];
    const json = await res.json();
            ...
            <main className="pt-20 pb-24">
                <div className="container mx-auto max-w-7xl px-4 sm:px-6 mb-6 sm:mb-8">
                    <nav className="flex items-center gap-1.5 text-xs text-gray-400 font-sans">
                        ...
                        <Link href="/case-study" className="hover:text-teal-600 transition-colors">Case Study</Link>
                        <ChevronRight className="h-3 w-3" />
                ...
                <div className="container mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 max-w-7xl">
                    <div className="lg:col-span-8">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <ShareButton title={article.title} text={article.excerpt.length ? article.excerpt[0]?.children?.[0]?.text : "Check out this case study"} className="text-gray-500 hover:text-teal-600 font-medium text-sm border border-gray-200 px-3 py-1.5 rounded-full bg-white hover:bg-gray-50 shadow-sm" />
                        </div>
