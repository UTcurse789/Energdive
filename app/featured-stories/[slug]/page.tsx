...
async function getRelated(tags: string[], currentSlug: string) {
    if (!tags.length) {
        const res = await fetch(
            `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][$eq]=Featured Stories&filters[slug][$ne]=${currentSlug}&pagination[limit]=4&populate=*&sort=publishedAt:desc`,
            { cache: "no-store" }
        );
        if (!res.ok) return [];
        const json = await res.json();
    ...
    const url = `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][$eq]=Featured Stories&${tagFilters}&filters[slug][$ne]=${currentSlug}&populate=*&pagination[limit]=4`;
...
export async function generateMetadata({
    ...
    return {
        ...
        openGraph: {
            title: shareTitle,
            description,
            url: `https://www.energdive.com/featured-stories/${slug}`,
            siteName: "Energdive",
            images: [
                {
                    url: imageUrl,
