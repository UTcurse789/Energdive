...
export default async function AnalysisDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const article = {
        ...
        tags: normalizedTags,
        category: attrs.type_of_content?.name || attrs.type_of_content?.data?.attributes?.name || "Analysis",
    };
            ...
            <main className="pt-20 pb-24">
                <div className="container mx-auto max-w-7xl px-4 sm:px-6 mb-6 sm:mb-8">
                    <nav className="flex items-center gap-1.5 text-xs text-gray-400 font-sans">
                        ...
                        <Link href="/analysis" className="hover:text-teal-600 transition-colors">Analysis</Link>
                        <ChevronRight className="h-3 w-3" />
                ...
                <div className="container mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 max-w-7xl">
                    <div className="lg:col-span-8">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <ShareButton title={article.title} text={article.excerpt.length ? article.excerpt[0]?.children?.[0]?.text : "Check out this analysis"} className="text-gray-500 hover:text-teal-600 font-medium text-sm border border-gray-200 px-3 py-1.5 rounded-full bg-white hover:bg-gray-50 shadow-sm" />
                        </div>
