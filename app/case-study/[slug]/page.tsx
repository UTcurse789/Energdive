...
async function getRelated(currentSlug: string) {
    try {
        const res = await fetch(
            `${STRAPI_BASE_URL}/api/contents?filters[type_of_content][name][\$eq]=Case Study&filters[slug][\$ne]=${currentSlug}&pagination[limit]=15&sort=publishedAt:desc`,
            { next: { revalidate: 60 } }
        );
        if (res.ok) {
            const json = await res.json();
