 ...
 *
 * Sends an OTP to the user's email for verification during onboarding Step 2.
...
export async function POST(req: Request) {
    try {
        ...
        if (!BREVO_API_KEY) {
            console.error("[EMAIL_OTP] BREVO_API_KEY not set");
            return NextResponse.json(
        ...
        console.log(`[EMAIL_OTP] Sending OTP to: ${normalizedEmail}, otp: ${otp}`);
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#0B0F19;padding:40px 20px;">
                    ...
                    <tr>
                        <td style="padding:48px 40px;">
                            ...
                            <p style="margin:0 0 24px;color:#4B5563;font-size:16px;line-height:1.7;">
                                Use the verification code below to complete your EnergClub registration.
                            </p>
        ...
        const res = await fetch(BREVO_API_URL, {
            ...
            body: JSON.stringify({
                ...
                subject: `${otp} — Your EnergDive Verification Code`,
                htmlContent,
            }),
        });
        ...
        if (!res.ok) {
            ...
            console.error(`[EMAIL_OTP] Brevo error: ${res.status} ${errorText}`);
            return NextResponse.json(
        ...
        console.log(`[EMAIL_OTP] OTP sent successfully to: ${normalizedEmail}`);
