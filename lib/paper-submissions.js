...
export function normalizePaperSubmission(item) {
    ...
    const finalPaperSubmissions = normalizeFinalPaperSubmissions(item, attrs);
    ...
    return {
        ...
        affiliation: attrs?.affiliation ?? attrs?.institution ?? "",
        profession: attrs?.Profession ?? attrs?.profession ?? "",
        ...
        pdf: attrs?.abstract_pdf ?? item?.abstract_pdf ?? attrs?.pdf ?? item?.pdf ?? null,
        finalPaperSubmissions,
        hasAcceptedFinalPaper: finalPaperSubmissions.some((paper) => ["accepted", "published", "approved"].includes((paper.finalStatus ?? "").toLowerCase())),
        finalPaperPdf: finalPaperSubmissions.find((paper) => ["accepted", "published", "approved"].includes((paper.finalStatus ?? "").toLowerCase()))?.fullPaper ?? null,
    };
}
...
export function normalizeAbstractSubmission(item) {
    const attrs = readStrapiAttributes(item);
    const rawSectors = toRelationArray(item?.sectors ?? attrs?.sectors);
    const { sectorNames, subSectorNames, subSectors, allSectorNames } = splitPaperTaxonomy(rawSectors);
    const finalPaperSubmissions = normalizeFinalPaperSubmissions(item, attrs);
    ...
    return {
        id:
            ...
            attrs?.documentId ??
            attrs?.slug ??
            `${attrs?.author_name ?? "abstract"}-${attrs?.submitted_date ?? attrs?.createdAt ?? "entry"}`,
        documentId: attrs?.documentId ?? item?.documentId ?? null,
        title: attrs?.title ?? "",
        authorName: attrs?.author_name ?? "",
        authorEmail: attrs?.author_email ?? "",
...
/**
 * Strapi v5 returns single media fields as an array (e.g. full_paper: [{url: "..."}]).
 * Strapi v4 and some v5 relations return them as a plain object (e.g. full_paper: {url: "..."}).
 * This helper normalises both shapes to always return a single media object or null.
 */
function normalizeMediaField(value) {
    if (!value) return null;
    if (Array.isArray(value)) {
        // Strapi v5 array shape — take the first element
...
function normalizeFinalPaperSubmissions(item, attrs) {
    const rawFinals = toRelationArray(item?.final_paper_submissions ?? attrs?.final_paper_submissions);

    return rawFinals.map((fItem) => {
        const fAttrs = readStrapiAttributes(fItem);
        const rawFullPaper = fAttrs?.full_paper ?? fItem?.full_paper ?? null;
        return {
            id: fItem?.id ?? fAttrs?.id ?? fAttrs?.documentId,
            documentId: fAttrs?.documentId ?? fItem?.documentId ?? null,
            title: fAttrs?.title ?? "",
            authorName: fAttrs?.author_name ?? "",
            authorEmail: fAttrs?.author_email ?? "",
            finalStatus: fAttrs?.final_status ?? "pending",
            finalSubmissionDate: fAttrs?.final_submission_date ?? fAttrs?.createdAt ?? "",
            fullPaper: normalizeMediaField(rawFullPaper),
        };
    });
}
